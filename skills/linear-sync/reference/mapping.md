...
| `ship` | attach the PR: `save_issue` BY id with `links:[{url:"<pr-url>",title:"PR"}]` on the feature's issues; `save_status_update` (project, body "PR open: <pr-url>"). The project state stays "In Progress" — Linear **projects** have no "In Review" state (that state lives on issues), and `save_project` with an unknown state **silently no-ops**, recording a transition that never happened. No merge transition — merge is out of scope for ship. |
