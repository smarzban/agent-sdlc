...
| `pr-review` | attach the PR: `save_issue` BY id with `links:[{url:"<pr-url>",title:"PR"}]` on the feature's issues; `save_status_update` (project, body "PR open: <pr-url>"). The project state stays "In Progress" — Linear **projects** have no "In Review" state (that state lives on issues), and `save_project` with an unknown state **silently no-ops**, recording a transition that never happened. No merge transition — merge is out of scope for pr-review. |

build and pr-review only **transition** entities the `plan` stage already created (milestones + `T-N`
issues, recorded in `linear-ids.json`); they never create — their transition steps live in the
`build` and `pr-review` skills.

**Keep the sync cheap and quiet.** Group a stage's writes into consecutive calls (milestones, then
issues) and batch where the API allows; do not narrate each round-trip. The sync is bookkeeping
beside the stage, never the stage's work.
