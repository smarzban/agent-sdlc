...
description: "Use when writing, overhauling, or reviewing a project's README (the front-door README.md of a repo). Produces a clear, scannable, source-grounded README that leads with what/why, adapts its section set to the repo type, keeps a lean quickstart distinct from full install, and links out to deeper docs instead of inlining them. Use also when a README \"isn't clear what the project is\" or has grown into a docs dump. Use this skill alone when only the README is wanted; writing-repo-docs produces the full essentials tree INCLUDING this front door (by this skill's method), and writing-technical-docs the internals. Triggers: \"write/fix the README\", \"README review\", \"the readme isn't clear\". Scope: standalone — for any repo's docs, independent of the Agent SDLC pipeline."
---
