...
- **The coverage ledger** — enumerate the **user-facing feature set** from the repo type's source of truth (`reference/structure-by-repo-type.md` maps each type to it: CLI → parser registrations, service → route table, library → exports, plugin → the skills/commands manifest) and the **existing usage pages**. This inventory is the ledger Phase 5 diffs against; it is what turns "usage/ covers every feature" from an assertion into a check. Treat any spec/pipeline/ADR/verification-report tree as **out-of-audience**, wherever rooted (root `specs/`, `docs/specs/`, `adr/`, `.agent-sdlc/`, …) — not features, not usage pages (see Phase 5).
...
- **Out-of-audience artifacts.** Spec/pipeline trees — any spec, ADR, pipeline, or verification-report tree, wherever rooted (root `specs/`, `docs/specs/`, `adr/`, `.agent-sdlc/`, …) — are build artifacts, not audience docs. Exclude them from the coverage ledger and never count them as usage pages; the link check still verifies a link that *points at* one, but a spec is never itself the thing being covered.
