...
export function parseDiff(diff) {
    ...
    for (const raw of diff.split("\n")) {
        ...
        if (raw.startsWith("\\")) {
            ...
            if (file)
                addedLines.push({ file, line: newLine, text: raw.slice(1) });
            newLine++;
        }
        else if (raw.startsWith("-")) {
...
const mk = (severity, area, title, file, line, rationale, suggestion) => ({
...
export function gitHygiene(cs) {
    ...
    for (const file of cs.files) {
        ...
        if (/(^|\/)node_modules\//.test(file)) {
            ...
            out.push(mk("high", "hygiene", "node_modules committed", "node_modules/", 0, "Vendored dependencies are committed to the repo.", "Remove node_modules from the change and add it to .gitignore."));
        }
        else if ((/^\.env(\.|$)/.test(base) || /\.env$/.test(base)) && !/\.(example|sample|template)(\.|$)/.test(base)) {
            out.push(mk("high", "hygiene", "environment file committed", file, 0, "A .env file may contain secrets and should not be committed.", "Remove it from the change, rotate any exposed secrets, and add it to .gitignore."));
        }
    }
    ...
    for (const a of cs.addedLines) {
        if (START_MARKER.test(a.text) || (SEP_MARKER.test(a.text) && filesWithStart.has(a.file))) {
            out.push(mk("critical", "hygiene", "merge conflict marker committed", a.file, a.line, CONFLICT_R, "Resolve the conflict and remove the <<<<<<< / ======= / >>>>>>> markers."));
        }
        if (TEST_FILE.test(a.file) && FOCUSED_TEST.test(a.text)) {
            out.push(mk("low", "test-coverage", "focused test left in", a.file, a.line, FOCUSED_R, "Remove the .only / f- prefix so the full suite runs."));
        }
        if (JS_FILE.test(a.file) && DEBUGGER.test(a.text)) {
            out.push(mk("medium", "hygiene", "debugger statement left in", a.file, a.line, DEBUGGER_R, "Remove the debugger statement."));
        }
    }
    return out;
}
/** The pure git-hygiene scanner wrapped in the async Scanner shape. Never calls a tool. */
export const gitHygieneScanner = {
    id: "git-hygiene",
...
const toRepoRelative = (p, repoDir) => (isAbsolute(p) ? relative(repoDir, p) : p).replace(/^\.\//, "");
...
const toolFailClosed = (id, reason) => ({
    findings: [mk("high", "hygiene", `${id} scan failed — failing closed`, "<scan>", 0, `The ${id} scan ran but did not complete cleanly (${reason}); a security scan that cannot complete must not silently pass.`, `Investigate the ${id} failure (oversized/pathological input, timeout, or tool error) and re-run.`)],
    warning: `${id}: ${reason}`,
});
const ranButFailed = (r, okCodes) => r.timedOut ? "timed out" : r.truncated ? "output exceeded the size cap"
...
const policyFinding = (file, what, envHint) => mk("high", "security", `${what}-scanner policy file changed`, file, 0, `This PR adds or modifies a ${what}-scanner policy/ignore file, which can suppress detections (ignore rules / allow-lists). Treat it as a privileged change.`, `Review it for hidden detections; prefer a trusted scanner config pinned outside the checkout (${envHint}).`);
...
function parseGitleaks(stdout, cs, repoDir) {
    const t = stdout.trim();
    if (!t)
        return { findings: [], rawHits: 0 }; // empty stdout = no leaks (clean)
    let hits;
    try {
        const p = JSON.parse(t);
        hits = Array.isArray(p) ? p : null;
    }
    catch {
        return null;
    }
    if (!hits)
        return null; // valid JSON but not the expected array shape ⇒ unparseable
    const changed = new Set(cs.files);
    const findings = hits
        .filter((h) => typeof h.File === "string")
        .map((h) => ({ rel: toRepoRelative(h.File, repoDir), h }))
        .filter(({ rel }) => changed.has(rel))
        .map(({ rel, h }) => mk("critical", "security", `secret: ${h.RuleID ?? "detected"}`, rel, Number(h.StartLine) || 0, `gitleaks matched ${h.RuleID ?? "a secret rule"}${h.Description ? ` (${h.Description})` : ""} — a credential committed in source.`, "Remove the secret, rotate it, and load it from config/env at runtime."));
...
export const secretsScanner = {
    ...
    async scan({ repoDir, changeset, run, signal }) {
        ...
        const r = await run("gitleaks", ["detect", "--no-banner", "--no-git", "--ignore-gitleaks-allow", "--gitleaks-ignore-path", ignorePath, "--report-format", "json", "--report-path", "/dev/stdout", "--source", repoDir, ...(cfg ? ["--config", cfg] : [])], repoDir, signal);
        if (r.missing)
            return { findings: policy, warning: "secrets: gitleaks not on PATH — skipped (install gitleaks to enable secret scanning)" };
        const failed = ranButFailed(r, [0, 1]); // gitleaks: 0 = no leaks, 1 = leaks found
        if (failed) {
            const fc = toolFailClosed("secrets", failed);
            return { findings: [...fc.findings, ...policy], warning: fc.warning };
        }
        const parsed = parseGitleaks(r.stdout, changeset, repoDir);
        if (!parsed) {
            const fc = toolFailClosed("secrets", "gitleaks output was not parseable JSON");
            return { findings: [...fc.findings, ...policy], warning: fc.warning };
        }
...
function parseOsv(stdout, manifests, repoDir) {
    const t = stdout.trim();
    if (!t)
        return []; // empty stdout = no vulns (clean)
    let parsed;
    try {
        parsed = JSON.parse(t);
    }
    ...
    for (const res of results) {
        ...
        if (!changed.has(path))
            continue; // exact match — only manifests this PR actually changed
        for (const pkg of Array.isArray(res?.packages) ? res.packages : []) {
            ...
            for (const v of Array.isArray(pkg?.vulnerabilities) ? pkg.vulnerabilities : []) {
                out.push(mk("high", "dependency", `${v?.id ?? "vulnerability"}: ${name}`, path, 0, `Known vulnerability in ${name}${version}${v?.summary ? ` — ${v.summary}` : ""}.`, `Upgrade ${name} to a patched version (see ${v?.id ?? "the advisory"}).`));
            }
        }
    }
    return out;
...
export const depsScanner = {
    ...
    async scan({ repoDir, changeset, run, signal }) {
        ...
        if (r.missing)
            return { findings: policy, warning: "deps: osv-scanner not on PATH — skipped (install osv-scanner to enable dependency CVE scanning)" };
        const failed = ranButFailed(r, [0, 1]); // osv: 0 = no vulns, 1 = vulns found
        if (failed) {
            const fc = toolFailClosed("deps", failed);
            return { findings: [...fc.findings, ...policy], warning: fc.warning };
        }
        const parsed = parseOsv(r.stdout, manifests, repoDir);
        if (!parsed) {
            const fc = toolFailClosed("deps", "osv-scanner output was not parseable JSON");
            return { findings: [...fc.findings, ...policy], warning: fc.warning };
        }
...
export const DEFAULT_SCANNERS = [gitHygieneScanner];
/** Default + external-tool adapters (each skips gracefully if its tool is absent). Used by the CLI. */
export const ALL_SCANNERS = [gitHygieneScanner, secretsScanner, depsScanner];
...
export async function runScan(repoDir, baseRef, opts = {}) {
    ...
    const failClosed = (reason) => ({
        output: {
            ...
            findings: [mk("high", "hygiene", "deterministic scan failed — failing closed", "<scan>", 0, `The deterministic scan could not complete (${reason}). A scan that cannot run is treated as blocking, not a pass.`, "Investigate the failure (oversized/pathological diff, git error, timeout, or an unsafe baseRef) and re-run.")],
        },
    ...
    if (!baseRef || /^-/.test(baseRef))
        return failClosed(`refusing unsafe baseRef "${baseRef}" (looks like a git option)`);
    ...
    try {
        ...
        const pin = (ref) => spawnGit(["rev-parse", ref], repoDir, ac.signal).then((s) => s.trim() || ref).catch(() => ref);
        ...
        const changeset = { files, addedLines: parsed.addedLines };
        const results = await Promise.all(scanners.map((s) => s.scan({ repoDir, changeset, run, signal: ac.signal })));
        const findings = results.flatMap((r) => r.findings);
        const warnings = results.map((r) => r.warning).filter((w) => Boolean(w));
