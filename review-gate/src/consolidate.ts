...
const titleTokens = (f: Finding): Set<string> =>
  new Set(
    f.title.toLowerCase().replace(/^\[[^\]]*\]\s*/, "").replace(/[^a-z0-9]+/g, " ").split(" ")
...
export function consolidate(outputs: ReviewerOutput[]): FindingCluster[] {
  ...
  const slug = (f: Finding) =>
    f.title.toLowerCase().replace(/^\[[^\]]*\]\s*/, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "x";
  ...
  for (const [file, items] of byFile) {
    ...
    const open: { items: Item[]; anchor: number }[] = [];
    for (const it of lined) {
      for (let k = open.length - 1; k >= 0; k--) {
        if (it.finding.line - open[k].anchor > LINE_WINDOW) { pushCluster(open[k].items, file); open.splice(k, 1); }
      }
      const g = open.find((grp) => sameIssue(grp.items[0].finding, it.finding));
      if (g) g.items.push(it);
      else open.push({ items: [it], anchor: it.finding.line });
    }
    for (const g of open) pushCluster(g.items, file);
  }
