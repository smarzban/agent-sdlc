...
`@AGENTS.md` import (preceded by a self-label comment), AGENTS.local.md, and a `.gitignore` under
which `git check-ignore AGENTS.local.md` exits 0 — and no other agent-instruction file.
...
2. **seed templates** — reference document(s) under `skills/repo-setup/reference/` holding the
   ...
   comment plus the one-line `@AGENTS.md` import; the AGENTS.local.md template is the
   what-belongs-here header.
