...
| AC-3 | reviewer-checked | Is the prune trigger mechanical and evaluable at a glance? PASS, T-1 (`fe4b3ca`) after the round's Critical. It shipped as "about a screen (order of 60-100 lines)", two hedges around a 40-line band, which the reviewer showed an unwilling agent can always read as inside normal range. Now a single integer (more than 100 lines), plus structure (current-state block not first) and content (an entry describing anything already merged, shipped, closed or decided). The overrun exception is bounded: it must name the leg it excuses and does not survive two consecutive updates. |
...
| AC-11 | reviewer-checked | Does build trigger an update at build entry, only when the doc exists, announced, never creating it? PASS, T-3 (`e1e6714`), judged independently of AC-13. Placement confirmed after the ready-to-build verdict and before the first dispatch. The round's sharper Important: the hook did not account for the build running in an isolated workspace, where a default-ignored doc does not exist, so it resolves against the working copy the run belongs to and skips loudly when it cannot. |
...
| AC-13 | reviewer-checked | Does ship trigger an update at its park step, under the same three conditions? PASS, T-3 (`e1e6714`), judged independently of AC-11. Both hooks also carry the conductor's ruling that a hook never asks: an undeterminable mode means skip with an announced reason, since both sit where the user is starting or finishing a long run. |
