...
- **`pr-review`** verifies green → writes, checker-verifies, and
  ...
  subagent when not. The owner judges the report. pr-review **never merges**.

## The artifact model

One sectioned spec file per feature; process reports live beside it, never inside it. Stages 1–5
each own and edit exactly one `##` section; `build` and `pr-review` never edit the spec.
...
**Spec lifecycle:** a shipped feature spec is an immutable snapshot (`pr-review` stamps a status header);
